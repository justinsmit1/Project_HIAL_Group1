...
class AWAC:
    ...
    def __init__(
        ...
        self.pi_optimizer = Adam(
            self.ac.pi.parameters(), lr=self.p_lr, weight_decay=1e-4
        )
        ...
        if env_name in data_envs:
            ...
            for file in data_envs[env_name]:
                if not os.path.exists(file):
                    warnings.warn(
                        colored(
                            "Offline data not found. Follow awac_data/instructions.txt to download. Running without offline data.",
                            "red",
                        )
                    )
                    break
    ...
    def test_agent(self):
        for j in range(self.num_test_episodes):
            ...
            while not (d or (ep_len == self.max_ep_len)):
                ...
                ep_len += 1
                #            self.logger.store(
                TestEpRet = ep_ret, TestEpLen = ep_len
            # Get unnormalized score
